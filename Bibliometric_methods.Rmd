...
• differences arise between the methods?
...
1. This section install and load relevant libraries
...
install_if_missing("gghighlight")
...
library(gtable)  # install.packages("gtables")
library(dplyr)
library(tidyr)
library(stringr)
library(ggplot2)
library(scales) # Needed for the labels = percent function
library(purrr)
...
items <- read.csv("kassel2526_items.csv")
...
items_authors <- read.csv("kassel2526_items_authors.csv")
...
authors_aff <- read.csv("kassel2526_authors_affiliations.csv") 
...
# 2. Group publications at article level and institution
# 3. 
# 4. 
# 5.  
#................................................................
...
# .............................................................
# Updating processed data with author sequence numbers
#................................................................
...
# Weights are calculated according to the author’s position and the total number of authors
# Example: publication with 3 authors. Raw weights: 1/1 (author 1), 1/2 (author 2), 1/3 (author 3). Sum = 1 + 0.5 + 0.333… = 1.833.
# Normalized weights: author 1 = 1 / 1.833 ≈ 0.545, author 2 = 0.5 / 1.833 ≈ 0.273, author 3 = 0.333 / 1.833 ≈ 0.182.
#...................................
...
# Each contributing author receives 𝟏/√N
# each institution receive ((𝟏/√N)*n)
...
```{r}
# Individual Countributions per organisation
table(merge_all$organization)
```
...
```{r}
# density function using author_count_

ggplot(df_to_save, aes(author_count, fill = organization, colour = organization)) +
...
```{r}
# density function using author_count_new 

ggplot(df_to_save, aes(author_count_new, fill = organization, colour = organization)) +
  geom_density(alpha = 0.3, na.rm = TRUE) + 
  #xlim(0, 55) + 
  theme(legend.position = "right",        # Legend on right side
        legend.title = element_text(size = 12, face = "bold"),
...
Models are used to explain behavior of each counting method
...
```{r}
# Generating a geometric series (doubling)
...
plot_data <- full_authorship %>%
  ...
  # 2. Reshape for plotting: 5 columns become 1 "Metric" column
  pivot_longer(
    cols = -author_count,           # Keep author_countas the X-axis
    names_to = "Method",       # Name of the legend column
    values_to = "Total_Credit" # Name of the Y-axis column
  )
...
ggplot(plot_data, aes(x = author_count, y = Total_Credit, color = Method)) +
  geom_line() +
  #facet_wrap(~ Method, scales = "free_y") + # Each method gets its own Y-axis scale
  geom_point() + # Optional: add points to see data nodes
  theme_minimal() +
  labs(
...
ggplot(plot_data, aes(x = author_count, y = Total_Credit, color = Method)) +
  ...
  theme_minimal() +
  labs(
    title = "Model 2: Co-Authorship with Only the 1st Author as External ",
    subtitle = "as authors increase from 1 to 512",
    x = "Author Count",
    y = "Score",
    color = "Counting Method"
...
ggplot(plot_data, aes(x = author_count, y = Total_Credit, color = Method)) +
  ...
  theme_minimal() +
  labs(
    title = "Model 3: Co-Authorship with only the 1st Author from the institution",
    subtitle = "as authors increase from 1 to 512",
    x = "Author Count",
    y = "Score",
    color = "Counting Method"
